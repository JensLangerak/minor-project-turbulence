...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 16 14:42:30 2018
...
test_discr = evolver.predict(sol, X_test)
...
meshRANS, U_RANS, gradU_RANS, p_RANS, gradp_RANS, tau_RANS, k_RANS, gradk_RANS, yWall_RANS, omega_RANS, S_RANS, Omega_RANS = RANS(case, Re_test[0], TurbModel, 7000, 140, 100)
...
plt.figure()
plt.title("Eigenvalue discrepancy GP %s_Re%i" % (case, Re_test[0]))
plt.plot(test_discr[0,:,:],test_discr[1,:,:],'b*')
plt.plot([0,1,0.5,0],[0,0,np.sin(60*(np.pi/180)),0],'k-')
plt.axis('equal')
plt.show()